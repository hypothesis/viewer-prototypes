(function () {
  ...
  function onlyComments(ann) {
    return ann.type === COMMENT;
  }

  function onlyAnnotations(ann) {
    return ann.type === ANNOTATION;
  }
  ...
  var annotations = Object.keys(cardsIndex).reduce(function (map, key) {
    ...
    map[key] = {
      ...
      set hidden(val) {
        hidden = val;
        this.cards.forEach(function (el) { el.hidden = hidden; });
      },
      get hidden() {
        return hidden;
      }
  ...
  if (commentHeading) {
    if (commentHeading.hasAttribute('collapsed')) {
      toggleAction = hideAnnotation;
    } else {
      toggleAction = showAnnotation;
    }
    annotationsList.filter(onlyComments).forEach(toggleAction);
  }
  ...
  document.documentElement.addEventListener('click', function (event) {
    ...
    if (event.target.hasAttribute('data-comment-heading')) {
      if (event.target.hasAttribute('collapsed')) {
        event.target.removeAttribute('collapsed');
        toggleAction = showAnnotation;
      } else {
        event.target.setAttribute('collapsed', '');
        toggleAction = hideAnnotation;
      }
      annotationsList.filter(onlyComments).forEach(toggleAction);
